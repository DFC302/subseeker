...
def crtsh():
	...
	# JSON Decode Error
		...
		if options().verbose:
			...
			print(colored(f"Parsing a domain with a large number of subdomains can sometimes crash results for crtsh.", "yellow")) 
			print(colored(f"Try again for possibly a different result or try using a sub keyword.\n", "yellow"))
			pass
		else:
			pass
...
def censys():
	...
	try:
		if options().api:
			...
			print(colored("Censys needs API credentials to be parsed.", "yellow"))
