...
'''
print(colored(f"\nYour search results have been written to: \
			{options().out}", "yellow"))
		grabCount()
'''
def crtsh():
	...
	try:
		if options().api:
			with open("api_info.json", "r") as f:
				jfile = json.load(f)
				api_token = jfile["certspotter"]["key"]
...
def certdb(api_token=False):
	...
	try:
		...
		if options().api:
			with open("api_info.json", "r") as f:
				jfile = json.load(f)
				api_token = jfile["certdb"]["key"]

		url = f"https://api.spyse.com/v1/subdomains?api_token={api_token} \
...
def censys():
	...
	try:
		if options().api:
			with open("api_info.json", "r") as f:
				jfile = json.load(f)
				uid = jfile["censys"]["uid"]
				api_token = jfile["censys"]["token"]
			...
			response = requests.post(api_url, json=params, auth=(uid, api_token))
			data = response.json()
			...
			if data:
				for row in data["results"]:
					...
					for line in CN:
						print(colored(re.findall(regex, line)[0][3:], "blue"))
...
def create():
	...
	if options().out:
		...
		if platform.system() == "Linux" or platform.system() == "Darwin":
			...
			print(colored(f"Your results have returned {stdoutdata} unique \
				subdomains\n", "yellow"))
